...
vector<vector<double>> outputRatios(vector<Node*> nodes, int num_inputs, int num_splitters, int num_outputs) {
    // Construct the vector of ratios, which will hold what each splitter outputs in terms of the outputs of the others
    // The [i][j] entry is the amount of node i's output depends on node j's output
    // Eventually, we want to reduce everything to dependencies on the inputs
    // Initially, this will just be the trivial "this splitter outputs what it outputs" vector
    ...
    for (int i = num_inputs; i < num_inputs + num_splitters + num_outputs; ++i) {
        ...
        for (int j = 0; j < nodes[i]->inputs.size(); ++j) {
            ...
            for (int k = 0; k < num_inputs + num_splitters + num_outputs; ++k) {
                // Check if this is an output node; if so, then don't divide output by the number of belts coming out
...
int main() {
    ...
    vector<vector<double>> flow = outputRatios({s0, s1, s2, s3, s4, s5, s6, s7, s8, s9, s10, s11, s12, s13, s14, s15, s16}, 1, 12, 4);
